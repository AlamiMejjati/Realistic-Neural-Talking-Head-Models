...
def generate_landmarks(frames_list, face_aligner):
    ...
    try:
        inp = torch.tensor(np.stack(frames_list, axis=0))
        inp = inp.transpose(1, 3).transpose(2, 3)
        lpreds = fa.get_landmarks_from_batch(inp)
...
persons_list = persons_list[:135000][::-1]
vid_id_list = vid_id_list[:135000][::-1]
vid_list = vid_list[:135000][::-1]
name_list = name_list[:135000][::-1]
num_vid = 0
