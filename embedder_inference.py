...
# lmks = np.concatenate([frame_mark_images[k][1] for k in range(len(frame_mark_images))], axis=1)
# ims = np.concatenate([frame_mark_images[k][0] for k in range(len(frame_mark_images))], axis=1)
# bin_landmarks = 255. - np.mean(lmks, axis=2)
# bin_landmarks = (bin_landmarks > 0.) * 1.
# composite = ims * (1 - bin_landmarks[:,:,None]) + lmks * bin_landmarks[:,:,None]
...
# torch.save({
#         'e_hat': e_hat_video
#         }, os.path.join(save_path, 'latent', path_to_e_hat_video))
torch.save({
        'e_hat': e_hat_images
        }, os.path.join(save_path, 'latent', path_to_e_hat_images))
print('...Done saving')